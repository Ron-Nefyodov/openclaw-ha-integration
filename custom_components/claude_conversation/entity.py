"""Base entity for Claude Conversation."""
...
def _convert_content(
    ...
    for content in chat_content:
        if isinstance(content, conversation.ToolResultContent):
            ...
            if not messages or messages[-1]["role"] != "user":
                messages.append(
                    MessageParam(role="user", content=content.content)
                )
            elif isinstance(messages[-1]["content"], str):
                messages[-1]["content"] = [
                    TextBlockParam(type="text", text=messages[-1]["content"]),
            ...
            if not messages or messages[-1]["role"] != "assistant":
                messages.append(MessageParam(role="assistant", content=[]))
            ...
            if isinstance(content.native, ThinkingBlock):
                ...
                if has_native := isinstance(messages[-1]["content"], str):
                    messages[-1]["content"] = [
                        TextBlockParam(type="text", text=messages[-1]["content"]),
                    ]
                messages[-1]["content"].append(  # type: ignore[attr-defined]
                    RedactedThinkingBlockParam(
                        type="redacted_thinking", data=content.native.data
                    )
                )
            if content.content:
                messages[-1]["content"].append(  # type: ignore[union-attr]
...
async def _transform_stream(
    ...
    """Transform the Anthropic response stream into HA format."""
    if stream is None:
...
class ClaudeBaseLLMEntity(Entity):
    ...
    def __init__(self, entry: ClaudeConfigEntry, subentry: ConfigSubentry) -> None:
        ...
        for _iteration in range(MAX_TOOL_ITERATIONS):
            try:
                ...
                raise HomeAssistantError(
                    f"Sorry, I had a problem talking to Claude: {err}"
                ) from err
